...
const SelectBox = ({data}) => {
    return (
        <div className={`${data.align === "" ? "col-md-6 col-12" : data.align}`}>
            <label className={`text-uppercase text-white ${classes.labelStyle}`}>{data.label}</label>
            <select
                className={`bg-transparent rounded-0 border-start-0 border-end-0 border-top-0 pb-2 ps-0 ${classes.placeholderStyle}`}
